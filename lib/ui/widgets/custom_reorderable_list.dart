...
/// An Edited version of flutter [SliverReorderableList] implementation with [setState] removed from some parts
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      if (_reverse) {
        ...
        if (itemStart <= proxyItemStart && proxyItemStart <= itemMiddle) {
          ...
          // we should swap the item with the gap and we are done looking for
          // the new index.
          if (_initialDragIndex == newIndex - 2) {
            newIndex = item.index - 1; // workaround/fix for when dragging item downwards, then to its original index again.
          } else {
            newIndex = item.index;
          }
          break;
