...
class Tooltip extends StatefulWidget {
  ...
  ///
  /// Only one of [message] and [richMessage] may be non-null.
  final InlineSpan Function()? richMessage;
...
class TooltipState extends State<Tooltip> with SingleTickerProviderStateMixin {
  ...
  String _buildTooltipMessage() => widget.message?.call() ?? widget.richMessage?.call().toPlainText() ?? '';
  ...
  void _handleStatusChanged(AnimationStatus status) {
    ...
    switch ((_isTooltipVisible(_animationStatus), _isTooltipVisible(status))) {
      ...
      case (false, true):
        ...
        SemanticsService.tooltip(_buildTooltipMessage());
  ...
  void _handlePointerDown(PointerDownEvent event) {
    ...
    switch (_triggerMode) {
      ...
      case TooltipTriggerMode.tap:
        final TapGestureRecognizer recognizer = _tapRecognizer ??= TapGestureRecognizer(debugOwner: this, supportedDevices: triggerModeDeviceKinds);
        recognizer
  ...
  void _handleMouseEnter(PointerEnterEvent event) {
    ...
    final List<TooltipState> tooltipsToDismiss = Tooltip._openedTooltips.where((TooltipState tooltip) => tooltip._activeHoveringPointerDevices.isEmpty).toList();
  ...
  Widget _buildTooltipOverlay(BuildContext context) {
    ...
    final BoxConstraints defaultConstraints = BoxConstraints(
      minHeight: widget.height ?? tooltipTheme.constraints?.minHeight ?? _getDefaultTooltipHeight(),
    );
