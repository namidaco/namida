...
export 'package:flutter/painting.dart'
    show AssetImage, ExactAssetImage, FileImage, FilterQuality, ImageConfiguration, ImageInfo, ImageProvider, ImageStream, MemoryImage, NetworkImage;
...
///
/// If the image is later used by an [ImageAdvanced] or [BoxDecoration] or [FadeInImage],
...
/// Signature used by [ImageAdvanced.frameBuilder] to control the widget that will be
/// used when an [ImageAdvanced] is built.
///
...
/// it will yield the same result as if [ImageAdvanced.frameBuilder] was null.
///
...
/// `ImageAdvanced` widget itself was created) and thus able to be painted immediately.
...
/// See also:
///
///  * [ImageAdvanced.frameBuilder], which makes use of this signature in the [ImageAdvanced]
///    widget.
typedef ImageFrameBuilder = Widget Function(BuildContext context, Widget child, int? frame, bool wasSynchronouslyLoaded);

/// Signature used by [ImageAdvanced.loadingBuilder] to build a representation of the
...
/// way and return the wrapped widget. If this builder returns `child` directly,
/// it will yield the same result as if [ImageAdvanced.loadingBuilder] was null.
///
...
/// See also:
///
///  * [ImageAdvanced.loadingBuilder], which makes use of this signature in the [ImageAdvanced]
///    widget.
...
/// Signature used by [ImageAdvanced.errorBuilder] to create a replacement widget to
...
/// specified:
///
///  * [ImageAdvanced.new], for obtaining an image from an [ImageProvider].
///  * [ImageAdvanced.asset], for obtaining an image from an [AssetBundle]
///    using a key.
///  * [ImageAdvanced.network], for obtaining an image from a URL.
///  * [ImageAdvanced.file], for obtaining an image from a [File].
///  * [ImageAdvanced.memory], for obtaining an image from a [Uint8List].
///
/// The following image formats are supported: {@macro dart.ui.imageFormats}
///
/// To automatically perform pixel-density-aware asset resolution, specify the
/// image using an [AssetImage] and make sure that a [MaterialApp], [WidgetsApp],
/// or [MediaQuery] widget exists above the [ImageAdvanced] widget in the widget tree.
///
...
///
/// {@tool snippet}
/// The [ImageAdvanced] Widget also provides several constructors to display different
/// types of images for convenience. In this example, use the [ImageAdvanced.network]
...
///
/// ```dart
/// ImageAdvanced.network('https://flutter.github.io/assets-for-api-docs/assets/widgets/owl-2.jpg')
/// ```
/// {@end-tool}
///
...
/// TickerMode(
///   enabled: !isPaused,
///   child: ImageAdvanced(image: myAnimatedGif),
/// ),
/// ```
/// {@end-tool}
///
...
///
/// The [ImageAdvanced.asset], [ImageAdvanced.network], [ImageAdvanced.file], and [ImageAdvanced.memory]
...
///  * [ImageAdvanced](dart-ui/ImageAdvanced-class.html), the class in the [dart:ui] library.
...
class ImageAdvanced extends StatefulWidget {
  ...
  /// [ImageAdvanced.network] and [ImageAdvanced.asset] respectively.
  ///
  ...
  /// `webHtmlElementStrategy` parameter of [ImageAdvanced.network] to display the image
  ...
  ImageAdvanced.file(
    ...
    int? cacheHeight,
       ...
       assert(
         !kIsWeb,
         'ImageAdvanced.file is not supported on Flutter Web. '
         'Consider using either ImageAdvanced.asset or ImageAdvanced.network instead.',
       ),
  ...
  ///
  /// ```dart
  /// ImageAdvanced.asset('images/cat.png')
  /// ```
  ///
  ...
  ///
  /// ```dart
  /// ImageAdvanced.asset('icons/heart.png', package: 'my_icons')
  /// ```
  /// {@end-tool}
  ///
  ...
  ImageAdvanced.asset(
    ...
    int? cacheHeight,
         ...
         cacheWidth,
         cacheHeight,
         scale != null ? ExactAssetImage(name, bundle: bundle, scale: scale, package: package) : AssetImage(name, bundle: bundle, package: package),
       ),
       loadingBuilder = null,
       assert(cacheWidth == null || cacheWidth > 0),
       assert(cacheHeight == null || cacheHeight > 0);
  ...
  /// formats like rawRgba (the default format of [dart:ui.ImageAdvanced.toByteData])
  ...
  ///
  /// {@template flutter.widgets.ImageAdvanced.frameBuilder.chainedBuildersExample}
  /// ```dart
  /// ImageAdvanced(
  ///   image: _image,
  ...
  ///
  /// If a [loadingBuilder] is specified for an image, the [ImageAdvanced] widget is
  ...
  /// consider the following builders used in conjunction:
  ///
  /// {@macro flutter.widgets.ImageAdvanced.frameBuilder.chainedBuildersExample}
  ///
  /// {@tool dartpad}
  ...
  ///
  /// {@tool dartpad}
  /// The following sample uses [errorBuilder] to show 'ImageAdvanced
  ...
  final double? Function(ImageInfo? info)? width;
  ...
  /// Consider using [fit] to adapt the image's rendering to fit the given width
  /// and height if the exact image dimensions are not known in advance.
  final double? Function(ImageInfo? info)? height;
  ...
  ///
  /// We have constructed a 'Person' widget that displays an avatar [ImageAdvanced] of
  /// the currently loaded person along with their name. We could request for a
  /// new person to be loaded into the widget at any time. Suppose we have a
  /// person currently loaded and the widget loads a new person. What happens
  /// if the [ImageAdvanced] fails to load?
  ///
  ...
  @override
  State<ImageAdvanced> createState() => _ImageState();
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(DiagnosticsProperty<Function>('width', width, defaultValue: null));
    properties.add(DiagnosticsProperty<Function>('height', height, defaultValue: null));
...
class _ImageState extends State<ImageAdvanced> with WidgetsBindingObserver {
  ...
  late DisposableBuildContext<State<ImageAdvanced>> _scrollAwareContext;
  ...
  void initState() {
    ...
    _scrollAwareContext = DisposableBuildContext<State<ImageAdvanced>>(this);
  }
  ...
  @override
  void didUpdateWidget(ImageAdvanced oldWidget) {
  ...
  void _resolveImage() {
    ...
    final w = widget.width?.call(_imageInfo);
    final h = widget.height?.call(_imageInfo);
  ...
  void _replaceImage({required ImageInfo? info}) {
    ...
    if (oldImageInfo != null) {
      SchedulerBinding.instance.addPostFrameCallback(
        ...
        debugLabel: 'ImageAdvanced.disposeOldInfo',
      );
    }
    _imageInfo = info;
  }
  ...
  void _stopListeningToStream({bool keepStreamAlive = false}) {
    ...
    // suppress errors.
    //
    // Reason: When an app provides an `ImageAdvanced` widget with an `errorBuilder`, it
  ...
  Widget build(BuildContext context) {
    ...
    result = RawImage(
      ...
      width: widget.width?.call(_imageInfo),
      height: widget.height?.call(_imageInfo),
