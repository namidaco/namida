...
class PlaylistController extends GetxController {
  ...
  void generateRandomPlaylist() {
    ...
    addNewPlaylist('$kPlaylistAutoGenerated ${l + 1}', tracks: rt);
  }
  ...
  Future<void> prepareAllPlaylistsFile() async {
    await for (final p in Directory(kPlaylistsFolderPath).list()) {
  ...
  Future<void> _prepareHistoryFile() async {
    final file = File(kHistoryPlaylistFilePath);
    await file.create();
  ...
  Future<void> _prepareFavouritesFile() async {
    final file = File(kFavouritesPlaylistFilePath);
    await file.create();
    final String content = await file.readAsString();
    if (content.isNotEmpty) {
      defaultPlaylists.add(Playlist.fromJson(jsonDecode(content)));
  ...
  Future<void> backupHistoryPlaylist() async {
    final defaultfile = File(kHistoryPlaylistFilePath);
    if (defaultfile.statSync().size > 2) {
      await defaultfile.copy(kHistoryPlaylistBackupFilePath);
    }
  }

  /// If a history backup was found, copies it and returns [true].
  Future<bool> tryRestoringHistory() async {
    final backupfile = File(kHistoryPlaylistBackupFilePath);
    if (await backupfile.exists()) {
      await backupfile.copy(kHistoryPlaylistFilePath);
      return true;
    }
    return false;
  }
  ...
  Future<void> _saveHistoryToStorage() async {
    await _saveDefaultPlaylistToStorageAndRefresh(kHistoryPlaylistFilePath, namidaHistoryPlaylist);
  }

  Future<void> _saveFavouritesToStorage() async {
    await _saveDefaultPlaylistToStorageAndRefresh(kFavouritesPlaylistFilePath, namidaFavouritePlaylist);
  }
  ...
  Future<void> _savePlaylistToStorageAndRefresh(Playlist playlist) async {
    _refreshStuff();
    await File('$kPlaylistsFolderPath${playlist.date}.json').writeAsString(jsonEncode(playlist.toJson()));
  }

  Future<void> _deletePlaylistFromStorageAndRefresh(Playlist playlist) async {
    _refreshStuff();
    await File('$kPlaylistsFolderPath${playlist.date}.json').delete();
  }
}
